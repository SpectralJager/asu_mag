...
func DivBy3() {
	perceptron := NewPerceptron(4, 0.5, 0.9)
	for range 1 {
		perceptron.Train(
			[][]float64{
			...
			[]float64{0.1, 0.9, 0.1, 0.9, 0.1, 0.9, 0.1, 0.9},
		)
	}
	fmt.Printf("training result: %+v\n", perceptron)
	fmt.Println(perceptron.Predict([]float64{0.9, 0.1, 0.1, 0.1, 0.1, 0.9}))
}
